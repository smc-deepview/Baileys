...
export const makeMessagesRecvSocket = (config: SocketConfig) => {
	...
	const msgRetryCache =
		...
		new NodeCache<number>({
			...
			useClones: false,
			maxKeys: 10000 // Limit to 10k retry entries to prevent memory leak
		})
	const callOfferCache =
		...
		new NodeCache<WACallEvent>({
			...
			useClones: false,
			maxKeys: 1000 // Limit to 1k call offers to prevent memory leak
		})
	...
	const identityAssertDebounce = new NodeCache<boolean>({ 
		stdTTL: 5, 
		useClones: false,
		maxKeys: 1000 // Limit to 1k identity assertions to prevent memory leak
	})
	...
	const handleMessage = async (node: BinaryNode) => {
		...
		try {
			...
			await messageMutex.mutex(async () => {
				...
				// Reactive 1:1 edit recovery. WhatsApp delivers a message edit as a
				// secret-encrypted message but does NOT give the sender's companion
				// the original 1:1 message's messageSecret, so we can't decrypt the
				// edit here (groups are unaffected — their secret IS delivered). Ask
				// our own primary device to re-send the edit
				// (PeerDataOperation PLACEHOLDER_MESSAGE_RESEND); the phone returns it
				// as a plaintext protocolMessage edit that flows through normal
				// capture. Fires ONLY when the secret is genuinely missing (so it
				// can't double with a successful decrypt) and is deduped by
				// placeholderResendCache.
				try {
					const sec = normalizeMessageContent(msg.message)?.secretEncryptedMessage
					if (
						sec?.secretEncType ===
							proto.Message.SecretEncryptedMessage.SecretEncType.MESSAGE_EDIT &&
						sec.targetMessageKey?.id &&
						msg.key?.id
					) {
						const targetMsg = await getMessage(sec.targetMessageKey)
						if (!targetMsg?.messageContextInfo?.messageSecret) {
							logger.info(
								{ editKey: msg.key, targetKey: sec.targetMessageKey },
								'secret 1:1 edit undecryptable — requesting placeholder resend of the edit'
							)
							requestPlaceholderResend(msg.key).catch(err =>
								logger.warn(
									{ err: (err as Error)?.message },
									'placeholder resend for secret edit failed'
								)
							)
						}
					}
					...
					logger.warn(
						...
						'secret-edit resend check failed'
					)
				}
